...
Pre- and post-commit hooks are applied at the [bucket](/riak/kv/2.1.4/learn/concepts/buckets) level,
...
Because hooks are defined at the bucket level, you can create [bucket types](/riak/kv/2.1.4/developing/usage/bucket-types)
...
documentation on [object deletion](/riak/kv/2.1.4/using/reference/object-deletion)) to determine whether a delete is
occurring.
...
- `fail` --- The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API](/riak/kv/2.1.4/developing/api/http)) along with a generic error message about why the write was blocked.
