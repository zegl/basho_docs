...
def do_fetch_archived_content()
  ...
  if (not all_dirs_present and any_dirs_present)
    Kernel.abort("ERRPR: The current state of the static/ directory is "\
                 "corrupt.\n"\
                 "       Please run \`git clean -xdf static/\` to clear out "\
                 "the malformed files, and re-run this deploy script.")
  elsif (not any_dirs_present)
    puts("Extracting archived_docs.basho.com.tar.bz2 (this may take some "\
         "of time)...")
    ...
    puts("    Archived content directory tree verified.\n"\
         ...
         "          As such, it is advisable to periodically clean out the static/ "\
         "directory that this archive is extracted into.\n"\
         "          To do so, please run \`git clean -xdf static/\`, and re-run "\
         "this deploy script.")
  end
end
